class Spree::Promotion::Actions::GiveStoreCredit < Spree::PromotionAction
  ...
  def perform(options = {})
    ...
    if _user = options[:user]
      _user.store_credits.create(:amount => preferred_amount, :remaining_amount => preferred_amount,  :store_credit_reason_id => preferred_reason_id, :store_credit_type_id => preferred_type_id, :expiry => expiry_days)
    end
  end
